  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (successCount === files.length) {
        ...
        setFiles(prev => prev.slice(successCount)); // Keep failed files
        onSuccess();
      } else {
        toast.error('Không có ảnh nào được tải lên. Hãy kiểm tra kết nối và cấu hình.', { id: toastId });
      }
    } catch (err) {
      toast.error('Đã xảy ra lỗi nghiêm trọng trong quá trình upload.', { id: toastId });
    } finally {
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 rounded-2xl bg-indigo-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition-all hover:bg-indigo-700 hover:scale-105 active:scale-95"
      >
        <Plus size={18} />
        ...
        {isOpen && (
          <div className="fixed inset-0 z-200 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl overflow-hidden rounded-4xl bg-white p-8 shadow-2xl dark:bg-slate-900 border border-slate-100 dark:border-slate-800"
            >
              <div className="mb-6 flex items-center justify-between">
                <h2 className="text-2xl font-black text-slate-900 dark:text-white">Upload Memories</h2>
                <button 
              ...
              <div
                {...getRootProps()}
                className={`group relative cursor-pointer rounded-4xl border-2 border-dashed p-10 text-center transition-all ${
                  isDragActive 
              ...
              >
                <input {...getInputProps()} />
                <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20">
                  <Upload size={32} />
                </div>
                <p className="text-sm font-bold text-slate-600 dark:text-slate-400">
                  {isDragActive ? 'Drop them here!' : 'Drag & drop images, or click to browse'}
                </p>
                <p className="mt-2 text-xs text-slate-400">Max 5MB per file (JPG, PNG, WEBP)</p>
              </div>
              ...
              <div className="mt-8 flex gap-4">
                <button
                  onClick={() => setIsOpen(false)}
                  disabled={isUploading}
                  className="flex-1 rounded-2xl border border-slate-100 bg-slate-50 py-4 text-sm font-black text-slate-900 transition-all hover:bg-slate-100 disabled:opacity-50 dark:border-slate-800 dark:bg-slate-800 dark:text-white"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-indigo-600 py-4 text-sm font-black text-white shadow-lg shadow-indigo-500/25 transition-all hover:bg-indigo-700 disabled:opacity-50"
                >
